...
resource "google_compute_instance" "copilot" {
  ...
  network_interface {
    subnetwork = var.use_existing_network ? var.subnetwork : google_compute_subnetwork.copilot_subnet[0].self_link
...
resource "google_compute_firewall" "copilot_firewall" {
  name          = each.key
  network       = var.use_existing_network ? var.network : google_compute_network.copilot_network[0].self_link
